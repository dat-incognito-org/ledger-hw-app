...
export async function calculateKeyImage (transport: Transport, coinPubKeyB64: string, encryptKmB64: string) {
    const coinPubKey = Buffer.from(coinPubKeyB64, "base64")
    const coinEncryptKm = Buffer.from(encryptKmB64, "base64")
    let buffer = Buffer.alloc(coinPubKey.length + coinEncryptKm.length)
    coinPubKey.copy(buffer, 0, coinPubKey.length)
    coinEncryptKm.copy(buffer,coinPubKey.length, coinEncryptKm.length)
